...
- **Text generation** — chat, tool calling, streaming with `agnes-3.0-flash`
- **Image understanding** — describe images with `agnes-3.0-flash` multimodal
- **Image generation** — text2img, img2img, multi-image compose with `agnes-image-2.5-flash`
- **Video generation** — text2video, img2video, keyframe animation with `agnes-video-2.5-flash`
...
# Generate a video
python scripts/agnes.py video text2video --prompt "Ocean waves at sunset, cinematic quality" --seconds 5
```
...
| `--model` | Model name (default `agnes-3.0-flash`) |
...
agnes image compose --prompt "Blend these images"   --image-url https://example.com/img1.jpg   --image-url https://example.com/img2.jpg

# Custom size and download directory
agnes image text2img --prompt "An oil painting" --size "1816x1024" --output-dir ./output
```
...
```bash
# Text-to-video (modern 2.5-flash with seconds duration, auto-poll)
agnes video text2video --prompt "A butterfly flying through a flower field" --seconds 5

# Image-to-video
agnes video img2video --prompt "Animate this image" --image-url https://example.com/photo.jpg --seconds 5

# Keyframe video (start and end frames)
agnes video keyframes --prompt "Animation sequence"   --image-url https://example.com/frame1.jpg   --image-url https://example.com/frame2.jpg --seconds 6

# Modern aspect ratio and resolution (Flash 720P)
agnes video text2video --prompt "Waterfall"   --seconds 6 --aspect-ratio 16:9 --size 720P   --download --output-dir ./videos

# Legacy frame-count mode (agnes-video-v2.0)
agnes video text2video --prompt "Ocean waves" --model agnes-video-v2.0   --width 1920 --height 1080 --num-frames 241 --frame-rate 30
...
| Argument | Description |
|----------|-------------|
| `mode` | Mode: `text2video` / `img2video` / `keyframes` / `reference` (required) |
| `--prompt` | Prompt text (required) |
| `--image-url` | Reference image URL(s), repeatable |
| `--audio-url` | Reference audio URL(s), repeatable |
| `--ref-video-url` | Reference video URL(s), repeatable |
| `--model` | Video model (default `agnes-video-2.5-flash`, or `agnes-video-2.5`, `agnes-video-v2.0`) |
| `--seconds` | Duration in seconds (default `5`, range 4–12) |
| `--aspect-ratio` | Aspect ratio (default `16:9`, choices: `16:9`, `9:16`, `1:1`, `4:3`, `3:4`, `21:9`) |
| `--size` | Resolution (default `720P`, choices: `720P`, `1080P`, `1K`, `2K`) |
| `--width` | Video width in pixels (legacy v2.0) |
| `--height` | Video height in pixels (legacy v2.0) |
| `--num-frames` | Number of frames (legacy v2.0, default 121, must satisfy 8n+1, max 441) |
| `--frame-rate` | Frames per second (legacy v2.0, default 24, range 1–60) |
| `--seed` | Random seed for reproducibility |
| `--num-inference-steps` | Number of denoising steps (legacy v2.0) |
| `--negative-prompt` | Things to avoid in the video (legacy v2.0) |
...
When a prompt contains non-ASCII characters, `agnes` automatically translates it to English using `agnes-3.0-flash` before sending it to the target model. Use `--no-translate` to disable.
...
Video generation polls for completion by default with real-time rendering progress updates (`0%` to `100%`). Pass `--download` to save the result locally.
...
| Date | Changes |
|------|---------|
| 2026-09-15 | Upgraded default models to next-gen Agnes AI: `agnes-3.0-flash`, `agnes-image-2.5-flash`, `agnes-video-2.5-flash`; added modern video generation (`--seconds`, `--aspect-ratio`, `--size`), real-time rendering progress bar, Flash 720P safeguard, and multi-modal reference support while preserving full legacy v2.0 compatibility |
